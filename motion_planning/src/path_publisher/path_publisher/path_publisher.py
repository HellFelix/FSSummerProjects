...
class PathPublisher(Node):
    ...
    def __init__(self):
        ...
        self._reset_publisher = self.create_publisher(
            msg_type=Odometry, topic="/reset", qos_profile=self._qos
        )
    ...
    def _timer_callback(self) -> None:
        ...
        if not self.init:
            self.init = True
            x0 = center_points[0].pose.position.x
            y0 = center_points[0].pose.position.y
            x1 = center_points[1].pose.position.x
            y1 = center_points[1].pose.position.y
            yaw = np.arctan2(y1 - y0, x1 - x0)

            odom_msg = Odometry()
            odom_msg.header.frame_id = self._frame_id
            odom_msg.header.stamp = self.get_clock().now().to_msg()
            odom_msg.pose.pose.position.x = x0
            odom_msg.pose.pose.position.y = y0
            odom_msg.pose.pose.position.z = 0.0
            odom_msg.pose.pose.orientation.x = 0.0
            odom_msg.pose.pose.orientation.y = 0.0
            odom_msg.pose.pose.orientation.z = np.sin(0.5 * yaw)
            odom_msg.pose.pose.orientation.w = np.cos(0.5 * yaw)

            self._reset_publisher.publish(odom_msg)
